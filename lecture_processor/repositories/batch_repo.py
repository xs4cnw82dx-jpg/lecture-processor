...
def list_batch_jobs_by_uid(db, uid, limit=100):
    query = apply_where(batch_jobs_collection(db), 'uid', '==', uid).order_by('created_at')
...
def list_active_batch_jobs(db, statuses, limit=50):
    ...
    if not active_statuses:
        return []
    query = apply_where(batch_jobs_collection(db), 'status', 'in', active_statuses).order_by('updated_at')
    if isinstance(limit, int) and limit > 0:
        query = query.limit(limit)
    return list(query.stream())
