(function () {
  ...
  async function createLink(){const role=$('dialog-content').querySelector('[data-field=linkRole]').value,days=+$('dialog-content').querySelector('[data-field=linkDays]').value,require_signin=$('dialog-content').querySelector('[data-field=linkSignin]').checked;const result=await api('/api/books/'+b.id+'/sharing',{method:'POST',body:JSON.stringify({role,days,require_signin})});$('share-result').innerHTML=`<label class="book-field"><span>Share this link</span><input id="share-url" readonly value="${esc(result.url)}"></label><button class="secondary-btn" id="copy-link">Copy link</button>`;const settings=await api('/api/books/'+b.id+'/sharing');$('existing-links').innerHTML=sharingLinks(settings.links);}
