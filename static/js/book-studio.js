(function () {
  ...
  function checkpoint(){if(!editable())return false;undoStack.push(M.clone({pages:b.pages,deletedPages:b.deletedPages,...metadata()}));if(undoStack.length>60)undoStack.shift();redoStack=[];return true;}
  ...
  function keepSpreadsTogether(){const result=M.preserveSpreads(b.pages);if(result.length>100){const previous=undoStack.pop();if(previous)Object.assign(b,previous);throw new Error('This change needs a blank page to keep the spread together. The book is at its page limit.');}b.pages=result;}
  ...
  function undo(redo=false){if(!editable())return;const from=redo?redoStack:undoStack,to=redo?undoStack:redoStack;if(!from.length)return;to.push(M.clone({pages:b.pages,deletedPages:b.deletedPages,...metadata()}));Object.assign(b,from.pop());if(!b.pages.some(p=>p.id===active))active=b.pages[0].id;selected=[];changed();}
  ...
  async function doExport(){
    ...
    try{
      ...
      download(blob,(snapshot.title||'Book')+(format==='editable'?' - editable':'')+'.'+(format==='pdf'?'pdf':'docx'));$('export-progress').textContent='Your book is ready. Check your downloads.';
